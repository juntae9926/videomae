...
class VideoClsDataset(Dataset):
    ...
    def __init__(self,anno_path, data_path, mode='train', clip_len=8,
        ...
        import pandas as pd
        anno = pd.read_csv(self.anno_path)
        self.dataset_samples = list(anno['video_id'])
        self.dataset_lengths = list(anno['duration_sec'])
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            sample_video = self.dataset_samples[index]
            sample = os.path.join(args.data_path, 'train/reference' ,sample_video)
            ...
            if args.num_sample > 1:
                ...
                augmented = self.data_transform(buffer) # input: ndarray
                buffer = self._aug_frame(buffer, args)

            return buffer, augmented, index
