...
def main(args):
    ...
    # if args.dist_eval:
    #     if len(dataset_val) % args.num_gpus != 0:
    #         print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
    #                 'This will slightly alter validation results as extra duplicate entries are added to achieve '
    #                 'equal num of samples per-process.')
    #     sampler_val = torch.utils.data.DistributedSampler(
    #         dataset_val, num_replicas=args.num_gpus, rank=0, shuffle=False)
    # else:
    ...
    #         assigner.get_scale if assigner is not None else None)
        ...
        #     args=args, model=model, model_parameters=optimizer_params, dist_init_required=True,
        # )
    ...
    criterion = losses.CrossBatchMemory(criterion, embedding_size=768, memory_size=1024)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            lr_schedule_values=lr_schedule_values, wd_schedule_values=0.05,
